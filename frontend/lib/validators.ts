...
export function validateEmail(email: string): string | null {
  ...
  // Strict whitelist: allow only letters, numbers, underscore for parts; only '@' and '.' as separators
...
export function validatePassword(password: string): string | null {
  ...
  // Strict whitelist: only letters, numbers, underscore, asterisk
...
export function validateMobile(mobile: string): string | null {
  ...
  // Allow only digits and optional leading '+'; max 11 digits
...
// --- Sanitizers (whitelist filtering for onChange) ---
...
export function sanitizePassword(input: string): string {
  // keep only letters, numbers, underscore, asterisk
  return (input || "").replace(/[^A-Za-z0-9_*]/g, "");
}

export function sanitizeMobile(input: string): string {
  let s = (input || "").replace(/[^0-9+]/g, "");
  // keep only leading '+' if present
  s = s.replace(/(?!^)\+/g, "");
